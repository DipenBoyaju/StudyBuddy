...
const QuizResultPage = () => {
  ...
  // const userAnswers = quizData?.userAnswers || quiz?.userAnswers || [];
      ...
      <div className="space-y-6">
...

        {
          detailedResults.map((result, index) => {
            const userSelectedText = result.selectedAnswer;

            const correctAnswerIndex = result.options.findIndex(opt => opt === result.correctAnswer);

            const userSelectedIndex = result.options.findIndex(opt => opt === userSelectedText);
                ...
                <div className="space-y-3 mb-4">
                  {
                    result.options.map((option, optIndex) => {
                      const isThisCorrectOption = optIndex === correctAnswerIndex;
                      const isThisWhatUserPicked = optIndex === userSelectedIndex;
                      // const isWrongAnswer = isThisWhatUserPicked && !isThisCorrectOption;

                      return (
                        <div key={optIndex} className={`relative px-4 py-3 rounded-lg border-2 transition-all duration-200 ${isThisCorrectOption ? 'bg-emerald-50 border-e-mist-300 shadow-lg shadow-emerald-500/10' : isThisWhatUserPicked ? 'bg-rose-50 border-rose-300' : 'bg-slate-50 border-slate-200'}`}>
                          <div className="flex items-center justify-between gap-3">
                            <span className={`text-sm font-medium ${isThisCorrectOption ? 'text-emerald-900' : isThisWhatUserPicked ? 'text-rose-900' : 'text-sky-700'}`}>
                              {option}
                            </span>
                            <div className="flex items-center gap-2">
                              ...
                              {
                                isThisWhatUserPicked && (
                                  <span className={`inline-flex items-center gap-1 px-2 py-1 border rounded-lg text-xs font-semibold ${isCorrect
                                    ? 'bg-emerald-100 border-emerald-300 text-emerald-700' // Green if right
                                    : 'bg-rose-100 border-rose-300 text-rose-700'         // Red if wrong
                                    }`}>
                                    {isCorrect ? (
                                      <CheckCircle2 className="w-3 h-3" strokeWidth={2.5} />
                                    ) : (
