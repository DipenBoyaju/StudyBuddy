  ...
  const renderMessage = (msg, index) => {
    ...
    return (
      <div key={index} className={`flex items-start gap-3 my-4 ${isUser ? 'justify-end' : ''}`}>
        ...
        <div className={`max-w-lg rounded-2xl shadow-sm p-2 ${isUser ? 'bg-linear-to-br from-emerald-500 to-teal-500 text-white rounded-br-md' : 'bg-white border border-slate-200/60 text-slate-800 rounded-bl-md'}`}>
          {
            isUser ? (
  ...
  return (
    <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
        ...
        <div ref={messagesEndRef} />
        {
          loading && (
            <div className="flex items-center gap-3 my-4">
              ...
              <div className="flex items-center gap-2 px-4 py-3 rounded-2xl rounded-bl-md bg-white border border-s-slate-200/60">
      ...
      <div className="p-5 border-t border-slate-200/60 bg-white/80">
        <form action="" onSubmit={handleSendMessage} className="flex items-center gap-3">
